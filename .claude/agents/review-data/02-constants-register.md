...
| Merchant speed (Teuton) | **12 fields/h** | `web/routes/distribution.py:141` `_TRIBE_MERCHANT_SPEED` | stock, **never measured on this server** | Wrong speed inflates travel → `sets_in_flight` → merchant counts, and can flip `over_budget`. One timed send settles it. |
...
| Stealable fallback | **0.67** | `raid_analyzer_service.py:376` `WAREHOUSE_RATIO` | pragmatic fallback when the carry icon is not parsed | **No cranny model exists.** If scoring was calibrated on hero-accompanied raids it over-estimates hero-less ones by up to 20% of cranny capacity — quiet |
