...
describe('the relay tier in the setup file', () => {
  it('rides the round trip, and the version rose for it', () => {
    ...
    // 6 since the relay tier landed (5 was the merchant cap, 4 `may_relay`, 3
    // the role templates). Pinned to a literal on purpose: the version has to
    // rise whenever a field is added, so that an older build refuses a file it
    // would otherwise half-load, and a literal is what makes forgetting the
    // bump a failing test rather than a tautology.
