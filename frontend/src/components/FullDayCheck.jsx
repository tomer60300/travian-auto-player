...
export default function FullDayCheck({ dayCheck, dayChecking, onRun, cropCeilings, villages }) {
  ...
  return (
    <div className="card p-4">
      ...
      {/* Section 6's third rule: everything home by 07:00. */}
      {dayCheck && <NightOverruns rows={dayCheck.night_overruns ?? []} />}
 ...
 * is.
 */
function NightOverruns({ rows }) {
  return (
    <div className="border-t-default pt-2 mb-3">
      <p className="text-xs font-semibold">
      ...
      {rows.length === 0 ? (
        <p className="text-success text-xs mt-1">
          Every night movement closes before the morning profile starts, so it begins with a
          full merchant pool everywhere.
        </p>
      ) : (
        <>
            ...
            <table className="w-full text-xs mt-1">
              <thead className="text-secondary uppercase">
                <tr>
                  ...
                  <th className="text-right px-2" title="Out and back. A merchant is committed for the whole journey.">
                    Round trip
                  </th>
                ...
                {rows.map((row) => (
                  ...
                  >
                    ...
                    <td className="text-right px-2 font-mono text-danger">
                      +{Math.round(row.overrun_minutes)}m
                    </td>
                  </tr>
                ))}
              </tbody>
