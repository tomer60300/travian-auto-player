...
export function useResumableOperation(opType, { onMessage, onStatusChange } = {}) {
  ...
  }, [closeWs, drainPendingStop, persistSessionId, setStatusBoth])
  ...
  }, [closeWs, drainPendingStop, persistSessionId, setStatusBoth, subscribeToExisting])
  ...
  // On mount: if there's a stored session_id, subscribe to it.
  useEffect(() => {
  ...
  useEffect(() => {
    const onVisible = () => {
      ...
      if (!live) subscribeToExisting()
    }
    const onPageShow = (e) => {
      // bfcache restores fire pageshow with persisted=true and no
      // visibilitychange. Treat the same.
      if (e.persisted && sessionIdRef.current) subscribeToExisting()
    }
