...
class ScoutReportData(BaseModel):
    ...
    stealable_resources: Dict[str, int] = Field(
        default_factory=dict, description="Stealable resources"
    )
...
class ScoutReport(Report):
    ...
    buildings_found: List[Dict[str, Any]] = Field(
        default_factory=list, description="Buildings found"
    )
...
class TradeReport(Report):
    ...
    resources_received: Dict[str, int] = Field(
        default_factory=dict, description="Resources received"
    )
