...
class TroopSendResult(BaseModel):
    ...
    troops_sent: Dict[str, int] = Field(default_factory=dict, description="Troops sent by type (t1-t10)")
...
class TroopComposition(BaseModel):
    ...
    def total(self) -> int:
        ...
        return sum([self.t1, self.t2, self.t3, self.t4, self.t5, self.t6, self.t7, self.t8, self.t9, self.t10, self.t11])
...
class ScoutResult(BaseModel):
    ...
    buildings_found: Optional[List[Dict[str, Any]]] = Field(None, description="Buildings discovered")
