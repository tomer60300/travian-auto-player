...
class Settings(BaseSettings):
    ...
    stealth_speed: float = Field(
        default=1.0, description="Speed multiplier: 0.5=fast, 1.0=normal, 2.0=cautious"
    )
    stealth_min_gap: float = Field(default=1.0, description="Min seconds between requests")
    stealth_max_gap: float = Field(default=2.5, description="Max seconds between requests")
    stealth_navigate: bool = Field(
    ...
    stealth_noise_rate: float = Field(
        default=0.08, description="Noise injection probability (0.0-1.0)"
    )
    stealth_max_daily_hours: float = Field(default=16.0, description="Maximum daily active hours")
    stealth_max_continuous_hours: float = Field(
        default=6.0, description="Max hours before forced break"
    )
    stealth_min_break_minutes: float = Field(
        default=10.0, description="Minimum break duration in minutes"
    )
    stealth_timing_variance: float = Field(
        default=0.4, description="Delay variance factor (0=tight, 1=very spread)"
    )

    # Live trade-route creation. The wire payload is verified against a captured
    # client request and pinned by tests, so this is an explicit opt-in rather
