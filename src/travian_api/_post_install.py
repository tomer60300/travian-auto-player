...
def main():
    ...
    print(f"This directory is not on your PATH.\n")
    ...
    if os.name == 'nt':
        ...
        if answer in ('', 'y', 'yes'):
            if add_to_user_path_windows(scripts_dir):
                print(f"[OK] Added to user PATH. Reopen your terminal and 'travian' will work.")
            else:
                print(f"Could not update PATH automatically.")
                print(f"Manual fix — add this to your PATH: {scripts_dir}")
        else:
            print(f"No problem. Use: python -m travian_api")
    else:
        shell = os.environ.get('SHELL', '/bin/bash')
        rc = '~/.bashrc' if 'bash' in shell else '~/.zshrc'
        print(f"Add to {rc}:  export PATH=\"$PATH:{scripts_dir}\"")
        print(f"Or use: python -m travian_api")
